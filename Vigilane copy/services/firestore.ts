...
export async function getUserSettings(uid: string): Promise<Settings> {
  try {
    ...
    if (__DEV__) console.warn('[firestore] getUserSettings failed:', err);
...
export async function saveUserSettings(uid: string, settings: Settings): Promise<void> {
  try {
    ...
    if (__DEV__) console.debug('[firestore] settings saved for', uid);
  } catch (err) {
    if (__DEV__) console.warn('[firestore] saveUserSettings failed:', err);
  }
}
